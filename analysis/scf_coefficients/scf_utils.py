...
class SCFvis:
    ...
    def fields_contour(self, grid_size, m, rs, snap, ngrid=128, field='acceleration', **kwargs):
        """
        ...
        if field == 'density' :
        ...
        elif field == 'density constrast' :
            rho = pot.density(xyz)
            pot_0 = gp.SCFPotential(m=m*u.Msun, r_s=rs*u.kpc, Snlm=S0, Tnlm=T0, units=galactic)
            rho_0 = pot_0.density(xyz)
            ax.contourf(x0, y0, (rho.value/rho_0.value).reshape(ngrid, ngrid)-1 , 20, cmap='inferno', 
            ...
            print(np.shape(acc), )
            acc_field = (linalg.norm(acc.value, axis=0)).reshape(ngrid, ngrid)
            ax.contourf(x0, y0, acc_field, 20, cmap='inferno', 
                        origin='lower', extent=[-250, 250, -250, 250], vmin=np.min(acc_field)*1.0,
                       vmax=np.max(acc_field)*0.8)
        ...
        ax.text(grid_size[0]*0.9, grid_size[0]*0.8, r'$t={:0.1f}\ $Gyr'.format(snap*0.02), c='w', fontsize=22)
